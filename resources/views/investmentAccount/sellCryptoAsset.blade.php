            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="custom-div flex justify-end">
                        <a href="{{route('showInvestmentAccount', $investmentAccount->id)}}" class="flex p-2 bg-gray-800 rounded-e-2xl">Back to account</a>
                    </div>
                    ...
                    <div class="custom-div">
                        @if(count($cryptoAssets))
                            ...
                            <form action="{{ route('investment-account.sell-crypto', $investmentAccount) }}" method="POST" class="border border-gray-300 p-6">
                                @csrf
                                <div class="flex justify-between">
                                    ...
                                    <select name="crypto_asset_id"  class="form-control" required>
                                        ...
                                        @foreach($cryptoAssets as $cryptoAsset)
                                            <option value="{{ $cryptoAsset->id }}">{{ $cryptoAsset->name }} ({{ $cryptoAsset->symbol }}) - quantity -
